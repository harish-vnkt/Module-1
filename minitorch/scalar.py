...
def central_difference(f, *vals, arg=0, epsilon=1e-6):
    ...
    new_vals = list(vals)
    assert arg < len(new_vals)
    forward_diff = new_vals[arg] + epsilon
    backward_diff = new_vals[arg] - epsilon
    new_vals[arg] = forward_diff
    forward_fn = f(*new_vals)
    new_vals[arg] = backward_diff
    backward_fn = f(*new_vals)
    return (forward_fn - backward_fn) / (2 * epsilon)
...
class Neg(ScalarFunction):
    ...
    def backward(ctx, d_output):
        # TODO: Implement for Task 1.4.
        # raise NotImplementedError('Need to implement for Task 1.4')
        return operators.neg(d_output) * d_output
